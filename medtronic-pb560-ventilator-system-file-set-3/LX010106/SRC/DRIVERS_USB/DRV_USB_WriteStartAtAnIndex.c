...
/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Filename  : DRV_USB_WriteStartAtAnIndex.c 											*/
/*																										*/
/*----------------------------------------------------------------------------*/
...
#include "DRV_USB_WriteStartAtAnIndex.h"

/******************************************************************************/
/*                   			FUNCTION BODY                    					*/
/******************************************************************************/
void DRV_USB_WriteStartAtAnIndex(UBYTE MsNb , 
...
/******************************************************************************/
	...
	CheckSum =		CheckSum
					...
					+ 	CMD_WRITE_FILE_AT_INDEX
					+	MsNb
					+	(UBYTE)((Index >> 24) & 0x00FF)
					+	(UBYTE)((Index >> 16) & 0x00FF)
					+	(UBYTE)((Index >> 8) & 0x00FF)
					+	(UBYTE)(Index & 0x00FF) ;
...
/******************************************************************************/
	...
	PEC_Emission[2] = CMD_WRITE_FILE_AT_INDEX ;
	PEC_Emission[3] = MsNb;
	...
	PEC_Emission[NbDataToWrite + 4] = (UBYTE)((Index >> 24) & 0x00FF) ;
	PEC_Emission[NbDataToWrite + 5] = (UBYTE)((Index >> 16) & 0x00FF) ;
	PEC_Emission[NbDataToWrite + 6] = (UBYTE)((Index >> 8) & 0x00FF) ;
	PEC_Emission[NbDataToWrite + 7] = (UBYTE)(Index & 0x00FF) ;

	/*%C Next byte is the CRC																	*/
	PEC_Emission[NbDataToWrite + 8] = (UBYTE) (CheckSum >> 8) ;
	PEC_Emission[NbDataToWrite + 9] = (UBYTE) CheckSum ;
