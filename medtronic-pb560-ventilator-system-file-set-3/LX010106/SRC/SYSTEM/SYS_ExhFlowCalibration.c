...
/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Filename  : SYS_ExhFlowCalibration.c 										 			*/
/*                                                                            */
/*----------------------------------------------------------------------------*/
...
#include "SYS_ExhFlowCalibration.h"
/******************************************************************************/
/*                            FUNCTION BODY                                   */
/******************************************************************************/
void SYS_ExhFlowCalibration(UWORD16 Request, t_SYS_CalibrationParameters CP)
{
...
/* Calibration array for insp/exh values filtering */
...
/*%C Check a difference to detect a calibration request */
	...
	{
/*%C 	Alarm state reset */
		DB_AlarmStatusWrite(TECH_ALARM_EXHAL_FLOW_OFFSET_DEFAULT_U16, ALARM_FALSE);
...
/*%C 5 & 20 lpm segment exhalation flow sensor */
...
/*%C Check if measured flow is out of range (Min & max) */
	if ( (FilterQi > ToleranceMore) 
     || (FilterQi < ToleranceLess) ) // conversion en 10ème de mbar ou de dl 
...
/*%C 	If the test fails */
		...
		{
/*%C 		Alarm activation */
			DB_AlarmStatusWrite(ALARM_CALIBRATION_FAIL_U16, ALARM_DETECTED);
/*%C 		Technical alarm activation */
