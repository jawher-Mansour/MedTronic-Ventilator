...
/*----------------------------------------------------------------------------*/
/*                                                                 			  	*/
/* Filename		: DB_IHM_Write_Adjust_Vol_Mode_Control_R.c  		        		   */
/*                                                           				  		*/
/*----------------------------------------------------------------------------*/
...
#include "DB_IHM_Write_Adjust_Vol_Mode_Control_R.h"
...
/*                                FUNCTION BODY		                          	*/
/******************************************************************************/
UWORD16 DB_IHM_Write_Adjust_Vol_Mode_Control_R(UWORD16 *Value)
{


/* Id values recuperation from adjust base in vol mode */
   UWORD32 Adjust_Vol_Control	= 
                     (UWORD32)EEP_DB_VOL_Adjust[ADJUST_VOL_CONTROL_U16];
   UWORD16 Adjust_High_R		= EEP_DB_VOL_Adjust[ADJUST_HIGH_R_U16];
   UWORD16 High_R_No_Select	= EEP_DB_VOL_Adjust[HIGH_R_NO_SELECT_U16];
      	/* Id values recuperation from adjust base in Pres mode */
   UWORD16 Disconnection_Delay_Auto =
   								 EEP_DB_VOL_Adjust[DISCONNECTION_DELAY_AUTO_U16];


/*%C Control R writing */
	DB_WriteDataInEepAndRam(&EEP_DB_VOL_Adjust[ADJUST_CONTROL_R_U16],
									*Value);						
...
/*%C Dependence test between Control R and Disconnection delay when AUTO */
	...
	{
		DB_WriteDataInEepAndRam(&EEP_DB_VOL_Adjust[ADJUST_DISCONNECTION_DELAY_U16],
								   (180 / *Value) * 1000);
	}
